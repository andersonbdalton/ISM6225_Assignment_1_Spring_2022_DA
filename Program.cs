        ...
        */
        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                String final_string = "";
                //create vowels list with both lower and upper case letters
                char[] vowels = { 'a', 'e', 'i', 'o', 'u', 'A', 'E', 'I', 'O', 'U' };
                //create blank string 
                string empty = "";

                
                //create a foreach loop to seach for vowels
                foreach (char c in vowels)
                {   //start at 0 and go to the length of the string
                    for (int i = 0; i < s.Length; i++)
                    {
                        //if a letter in the string is equal to a letter in the vowels list then empty the that letter with nothing 
                        if (s[i] == c)
                        {
                            empty += "";
                        }
        ...
        {
            try
                //create two strings using sb
            {StringBuilder bull1 = new StringBuilder(), bull2 = new StringBuilder();
                //use a foreach loop to look at each letter in the string 
                foreach (var item in bulls_string1)
                {
                    bull1.Append(item);
                }
                foreach (var item in bulls_string2)
                {
                    bull2.Append(item);
                }
                return bull1.ToString() == bull2.ToString();
            }
            catch (Exception)
            {
        ...
        */
        ...
        private static int SumOfUnique(int[] bull_bucks)
        {
            try
            {
                ...
                //look for unique ints in teh bull_bucks then store the count into uniqueBulls
        ...
        */
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            try
            {
                ...
                // i is increasing and moving to the right and b is decreasing and moving to the left
                for (int i = 0, b = bulls_grid.Length - 1;
                    i < bulls_grid.Length; i++, --b)
                {
                    //gobal counter;
                    bullsCount += i;
...
private static string RestoreString(string bulls_string, int[] indices)
        {
            try
            {   //make a new string the length of the bulls_string
                char[] bull_ans = new char[bulls_string.Length];
                // go to the length of the indices (numbers) then find that number in the bulls string and add it to the string
                for (int i = 0; i < indices.Length; i++)
                    bull_ans[indices[i]] = bulls_string[i];

                return new string(bull_ans);
        ...
        */
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            ...
            {   //character ch does not exist in word, do nothing
                if (string.IsNullOrEmpty(bulls_string6))
                    return "";
                //if the element in the ch bin is at index 0 return bull_strings6
                int index = bulls_string6.IndexOf(ch);
                if (index < 0) return bulls_string6;

                //reverse the segment of word that starts at index 0 and ends at the index of the first occurrence of ch
                var chars = bulls_string6.ToCharArray();
